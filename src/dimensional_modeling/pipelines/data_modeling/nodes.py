...
def get_dimension_ubicaciones(est_manz: pd.DataFrame, nom_domc: pd.DataFrame, lote_pred: pd.DataFrame) -> pd.DataFrame:
    ...
    dimension_ubicaciones_df = est_manz
    dimension_ubicaciones_df['comuna'] = dimension_ubicaciones_df.barrio.astype('int').astype(str).str[-2:]
    dimension_ubicaciones_df['barrio'] = dimension_ubicaciones_df.barrio.astype('int').astype(str).str[-2:]
    dimension_ubicaciones_df['manzana'] = dimension_ubicaciones_df.id_ubicacion.astype('int').astype(str).str[-3:]
    dimension_ubicaciones_df.drop_duplicates(inplace=True)
    dimension_ubicaciones_df['id_ubicacion'] = dimension_ubicaciones_df.id_ubicacion.astype('int').astype(str)

    dimension_ubicaciones_df = dimension_ubicaciones_df.merge(nom_domc,
                                                              how='inner',
                                                              on=['comuna', 'barrio'])
...
def get_hecho_manzanas(aux_estrato: pd.DataFrame,
                       dim_estrato: pd.DataFrame,
                       dim_dest: pd.DataFrame,
                       est_manz: pd.DataFrame) -> pd.DataFrame:
    aux_estrato = (
