...
def clean_estadisticas_por_manzana(est_manz: pd.DataFrame) -> pd.DataFrame:
    est_manz_df = est_manz[
        ['CANT_PREDIOS', 'POBLACION_APROX', 'OBJECTID', 'COBAMA', 'DESTINACION', 'COMUNA', 'BARRIO']].rename(
        columns={'CANT_PREDIOS': 'cant_predios', 'POBLACION_APROX': 'poblacion_aprox', 'OBJECTID': 'id',
                 'COBAMA': 'id_ubicacion', 'DESTINACION': 'destinacion', 'COMUNA': 'comuna', 'BARRIO': 'barrio'})

    est_manz_df = est_manz_df[est_manz_df['barrio'].notna()]
    est_manz_df = est_manz_df[est_manz_df['comuna'].notna()]

    est_manz_df[
        'poblacion_aprox_por_predio'] = est_manz_df.poblacion_aprox / est_manz_df.cant_predios
    est_manz_df['poblacion_aprox_por_predio'] = est_manz_df[
        'poblacion_aprox_por_predio'].apply(np.round)

    return est_manz_df

def clean_nomenclatura_domiciliaria(nom_domi: pd.DataFrame) -> pd.DataFrame:
    rename_cols = {'CODIGO_BARRIO': 'barrio', 'NOMBRE_BARRIO': 'nombre_barrio', 'CODIGO_COMUNA': 'comuna',
...
def clean_lote_del_predio(lote_predio: pd.DataFrame) -> pd.DataFrame:
    ...
    lote_del_predio_df = (lote_del_predio_df_raw
      ...
      .first()
      .rename(columns={'LONGITUD': 'longitud', 'LATITUD': 'latitud'})[['id_ubicacion', 'latitud', 'longitud']]
    )
...
def auxiliar_estrato(estrato_socioecono: pd.DataFrame) -> pd.DataFrame:
    ...
    auxiliar_estrato_df['id_ubicacion'] = \
        auxiliar_estrato_df.MANZANA.fillna(auxiliar_estrato_df.CODIGO_BARRIO).astype(str).str.split('.').str[0]
